...
const KG_PLATFORM_FILE = process.env.KG_PLATFORM_FILE || path.join(__dirname, '.kg-platform');

let kgCookie = '';
let kgLoginInfo = { loggedIn: false, nickname: '酷狗音乐', userId: '', avatar: '', vipType: 0, vipLevel: 'none', isVip: false };
try { if (fs.existsSync(KG_COOKIE_FILE)) kgCookie = fs.readFileSync(KG_COOKIE_FILE, 'utf8').trim(); }
catch (e) { kgCookie = ''; }
// 恢复平台设置（避免重启后用错误的 appid 请求）
try { if (fs.existsSync(KG_PLATFORM_FILE)) process.env.platform = fs.readFileSync(KG_PLATFORM_FILE, 'utf8').trim(); }
catch (e) {}
function saveKGPlatform(mode) {
  process.env.platform = mode === 'lite' ? 'lite' : '';
  try { fs.writeFileSync(KG_PLATFORM_FILE, process.env.platform || 'standard'); } catch (e) {}
}
// 会话级缓存：保证同一进程内设备指纹一致（声明前置，在函数使用前初始化）
let _kgCachedGuid = '';
let _kgCachedMid = '';
let _kgCachedDfid = '';
...
function saveKGCookie(c) {
  ...
  // 确保持久化的 cookie 包含设备标识（GUID/MID/dfid），下次启动时保持一致
  const obj = parseCookieString(kgCookie);
  if (!obj.KUGOU_API_GUID && _kgCachedGuid) obj.KUGOU_API_GUID = _kgCachedGuid;
  if (!obj.KUGOU_API_MID && _kgCachedMid) obj.KUGOU_API_MID = _kgCachedMid;
  if ((!obj.dfid || obj.dfid === '-') && _kgCachedDfid) obj.dfid = _kgCachedDfid;
  // 重新序列化以确保包含所有字段
  const keys = Object.keys(obj);
  if (keys.length) {
    kgCookie = keys.filter(k => obj[k]).map(k => k + '=' + obj[k]).join('; ');
  }
  try { fs.writeFileSync(KG_COOKIE_FILE, kgCookie); } catch (e) {}
}
function updateKGLoginInfoFromCookie() {
  const obj = parseCookieString(kgCookie);
  // 同步缓存中的设备标识
  ...
  if (obj.token && obj.userid) {
    kgLoginInfo = {
      ...
      userId: obj.userid || '',
      avatar: obj.avatar || '',
      vipType: parseInt(obj.vip_type || '0', 10) || 0,
      vipLevel: obj.vip_type > 0 ? 'vip' : 'none',
      isVip: parseInt(obj.vip_type || '0', 10) > 0,
    };
  }
}
updateKGLoginInfoFromCookie();
// 启动时如果已登录，延迟初始化（注册设备 + 获取用户信息）
if (kgLoginInfo.loggedIn) {
  setTimeout(() => {
    kgRegisterDev().then(dfid => { if (dfid) console.log('[KG Init] dfid ready:', dfid); }).catch(() => {});
    ensureKGUserInfo();
  }, 1000);
}
...
async function handleDiscoverHome() {
  const info = await getLoginInfo();
  const neteaseLoggedIn = !!(info && info.loggedIn);
  const kgLoggedIn = !!(kgLoginInfo && kgLoginInfo.loggedIn);
  const qqInfo = await getQQLoginInfo().catch(() => ({}));
  const qqLoggedIn = !!(qqInfo && qqInfo.loggedIn);

  // 没有任何平台登录 → 返回 starter 模式
  if (!neteaseLoggedIn && !kgLoggedIn && !qqLoggedIn) {
    return {
      loggedIn: false,
      user: null,
      dailySongs: [],
  ...
  const kgTasks = [];
  if (kgLoggedIn) {
    // 酷狗：获取用户歌单 + 热门搜索作为每日推荐
    const kgCookie = kgCookieObj();
    const kgUserid = kgCookie.userid || kgLoginInfo.userId || 0;
    const kgToken = kgCookie.token || '';
    kgTasks.push(
      kugouRequest({
        url: '/v7/get_all_list',
        method: 'POST',
        data: {
          userid: Number(kgUserid),
          token: kgToken,
          total_ver: 979,
          type: 2,
          page: 1,
        ...
        params: { plat: 1, userid: Number(kgUserid), token: kgToken },
        encryptType: 'android',
        cookie: kgCookie,
    ...
    // 用酷狗热门搜索生成推荐歌曲
    const hotKeywords = ['热歌', '新歌', '推荐', '流行', '经典'];
    kgTasks.push(
      kugouRequest({
        url: '/v3/search/song',
        method: 'GET',
        params: {
          keyword: hotKeywords[Math.floor(Math.random() * hotKeywords.length)],
          page: 1, pagesize: 12,
          albumhide: 0, iscorrection: 1, nocollect: 0,
  ...
  if (kgTasks.length) {
    tasks.push(Promise.all(kgTasks));
  } else {
    tasks.push(Promise.resolve([]));
  }

  // QQ 搜索推荐（简化）
  if (qqLoggedIn && !neteaseLoggedIn && !kgLoggedIn) {
    tasks.push(Promise.resolve([]));
  } else {
    tasks.push(Promise.resolve([]));
  }
  ...
  if (neteaseLoggedIn && result[0].length >= 4) {
    const neResult = result[0];

    const personalizedBody = neResult[0].status === 'fulfilled' && neResult[0].value && neResult[0].value.body || {};
    publicPlaylists = (personalizedBody.result || personalizedBody.data || [])
      .map(pl => mapDiscoverPlaylist(pl, '推荐歌单'))
      .filter(pl => pl.id && pl.name)
      .slice(0, 8);

    const podcastBody = neResult[1].status === 'fulfilled' && neResult[1].value && neResult[1].value.body || {};
    const podcastRaw = podcastBody.djRadios || podcastBody.djradios || podcastBody.radios || podcastBody.data || [];
    podcasts = (Array.isArray(podcastRaw) ? podcastRaw : [])
      .map(mapPodcastRadio)
      .filter(p => p.id && !isLowSignalPodcastItem(p))
      .slice(0, 6);

    if (neResult[2].status === 'fulfilled' && neResult[2].value) {
      const body = neResult[2].value.body || {};
    ...
    if (neResult[3].status === 'fulfilled' && neResult[3].value) {
      const body = neResult[3].value.body || {};
  ...
  // --- 解析酷狗结果 ---
  if (kgLoggedIn && result[1] && result[1].length >= 2) {
    // 酷狗歌单 → 转为统一格式
    const kgPlaylistResult = result[1][0];
    if (kgPlaylistResult) {
      const kgData = (kgPlaylistResult.data && kgPlaylistResult.data.data) || {};
      const kgList = (kgData.list || kgData.info || []);
      const kgPlaylists = kgList.slice(0, 6).map(pl => ({
        id: pl.global_collection_id || pl.listid || pl.id || '',
        listid: pl.listid || pl.special_id || '',
        name: pl.name || pl.special_name || pl.listname || '',
        cover: pl.imgurl || pl.pic || pl.cover || '',
      ...
      // 将酷狗歌单合并到推荐歌单
      publicPlaylists = publicPlaylists.concat(kgPlaylists).slice(0, 10);
    }

    // 酷狗搜索 → 每日推荐歌曲
    const kgSearchResult = result[1][1];
    if (kgSearchResult && dailySongs.length === 0) {
      const kgSearchData = (kgSearchResult.data && kgSearchResult.data.data) || {};
      const kgSearchLists = (kgSearchData.lists || kgSearchData.info || []);
      dailySongs = kgSearchLists.slice(0, 12).map(mapKGSong).filter(s => s.id && s.name);
    }
  }

  // 确定当前用户信息
  const activeUser = neteaseLoggedIn
    ? { userId: info.userId, nickname: info.nickname || '', avatar: info.avatar || '' }
    : kgLoggedIn
      ? { userId: kgLoginInfo.userId, nickname: kgLoginInfo.nickname || '', avatar: kgLoginInfo.avatar || '' }
      : null;
...
function audioProxyHeadersFor(audioUrl, range) {
  ...
  try {
    ...
    } else if (host.includes('qq.com') || host.includes('qpic.cn')) {
...
/** 调用 register_dev 获取服务端验证的真实 dfid */
async function kgRegisterDev() {
  const cookieObj = kgCookieObj();
  const userid = cookieObj.userid || kgLoginInfo.userId || 0;
  const token = cookieObj.token || '';
  const guid = cookieObj.KUGOU_API_GUID || '';
  ...
  const p = kugou.rsaEncrypt2({ aes: aesEncrypt.key, uid: userid, token });

  try {
    const result = await kugou.createRequest({
    ...
    // 解密响应
    const decrypted = kugou.playlistAesDecrypt({
      str: Buffer.from(result.body).toString('base64'),
      key: aesEncrypt.key,
    });

    if (decrypted && decrypted.status === 1 && decrypted.data && decrypted.data.dfid) {
      const dfid = decrypted.data.dfid;
      _kgCachedDfid = dfid;
      // 持久化到 cookie 文件
      const currentObj = parseCookieString(kgCookie);
      currentObj.dfid = dfid;
      const keys = Object.keys(currentObj);
      saveKGCookie(keys.filter(k => currentObj[k]).map(k => k + '=' + currentObj[k]).join('; '));
      console.log('[KGRegisterDev] dfid obtained:', dfid);
      return dfid;
    }
    console.warn('[KGRegisterDev] no dfid in response');
    return null;
  } catch (e) {
    console.warn('[KGRegisterDev] failed:', (e.body && e.body.error_code) || e.message);
    return null;
  }
}

/** 后台获取歌单封面（/v7/get_all_list 不返回封面，通过 detail API 补充） */
async function kgFetchPlaylistCovers(ids) {
  try {
    const cookieObj = kgCookieObj();
    const r = await kugouRequest({
      url: '/v3/get_list_info', method: 'POST',
      data: { data: ids.map(s => ({ global_collection_id: s })), userid: Number(cookieObj.userid || 0), token: cookieObj.token || '' },
      encryptType: 'android', cookie: cookieObj,
      headers: { 'x-router': 'pubsongs.kugou.com' },
    });
    const body = (r && r.data) || {};
    // /v3/get_list_info 返回 { data: [{...}, {...}] }（数组），每个元素有 global_collection_id
    const raw = body.data || body.info || [];
    const map = {};
    (Array.isArray(raw) ? raw : Object.values(raw)).forEach(pl => {
      if (pl && pl.global_collection_id) {
        map[pl.global_collection_id] = (pl.pic || pl.imgurl || pl.img || pl.create_user_pic || '').replace('{size}', '400');
      }
    });
    return map;
  } catch (e) { return null; }
...
/** 解析 kgCookie 字符串为对象，所有请求保持设备指纹一致
 *  模拟原始 KuGouMusicApi 中间件的设备标识注入行为 */
function kgCookieObj() {
  // 桌面版中 main.js 通过 saveKGCookie 写文件，server.js 的 kgCookie 不会同步。
  // 每次调用时重新读取文件，确保登录状态实时生效。
  try { if (fs.existsSync(KG_COOKIE_FILE)) { const fresh = fs.readFileSync(KG_COOKIE_FILE, 'utf8').trim(); if (fresh) kgCookie = fresh; } } catch (_) {}
  // 如果文件中有 token 但 kgLoginInfo 未标记已登录，同步更新
  if (!kgLoginInfo.loggedIn && kgCookie && kgCookie.includes('token=')) {
    updateKGLoginInfoFromCookie();
    // 延迟拉取用户信息（异步，不阻塞请求）
    setTimeout(() => { if (!kgLoginInfo.nickname || kgLoginInfo.nickname === '酷狗用户') ensureKGUserInfo(); }, 500);
  }
  const obj = parseCookieString(kgCookie);

  // GUID：原始 KuGouMusicApi 使用 cryptoMd5(getGuid()) 格式（32位hex），而非原始 UUID
  if (obj.KUGOU_API_GUID) {
    _kgCachedGuid = obj.KUGOU_API_GUID;
  } else if (_kgCachedGuid) {
    obj.KUGOU_API_GUID = _kgCachedGuid;
  } else {
    _kgCachedGuid = kugou.cryptoMd5(kugou.getGuid());
    obj.KUGOU_API_GUID = _kgCachedGuid;
  }

  // MID：从 GUID 计算
  if (obj.KUGOU_API_MID) {
    _kgCachedMid = obj.KUGOU_API_MID;
  } else if (_kgCachedMid) {
    obj.KUGOU_API_MID = _kgCachedMid;
  } else {
    _kgCachedMid = kugou.calculateMid(obj.KUGOU_API_GUID);
    obj.KUGOU_API_MID = _kgCachedMid;
  }

  // dfid：设备指纹 ID
  if (obj.dfid && obj.dfid !== '-') {
    _kgCachedDfid = obj.dfid;
  } else if (_kgCachedDfid) {
    obj.dfid = _kgCachedDfid;
  } else {
    _kgCachedDfid = kugou.randomString(24);
    obj.dfid = _kgCachedDfid;
  }

  // 补充原始 KuGouMusicApi 中间件注入的其他设备标识（缺则补）
...
/** SSA 二次验证：调用 verify_user_info 接口 */
async function kgHandleSSA(mid, userid, dfid, ssaCode, edt, sid, cookieObj) {
  try {
    const webglHash = cookieObj.KUGOU_API_WEBGL || kugou.generateWebGLHash();
    // 始终重新生成 simulate 数据（error body 中的可能已过期）
    const sim = kugou.generateSimulate(mid, userid, dfid, webglHash);
    const encrypt = kugou.cryptoAesEncrypt({});
    // verify 端点使用平台对应的 RSA 公钥
    const pk = kugou.cryptoRSAEncrypt({ key: encrypt.key });
    const verifyRes = await kugou.createRequest({
      ...
      data: {
        ...
        wasm: 1,
        i: '',
        sid: decodeURIComponent(sim.sid),
        edt: decodeURIComponent(sim.edt),
        verifycode: '',
        pk,
        params: encrypt.str,
      },
    ...
    console.warn('[KGSSA] verify failed:', (e.body && e.body.error_code) || e.message);
    return null;
  }
}

/** 调用酷狗 API（封装 kugou-core 的 createRequest，自动处理 SSA 二次验证） */
async function kugouRequest(config) {
  const cookieObj = config.cookie || kgCookieObj();
  try {
    const result = await kugou.createRequest({
      ...config,
      cookie: cookieObj,
    });
    return { data: result.body, headers: result.headers, cookie: result.cookie };
  } catch (err) {
    const e = err || {};
    const body = e.body || {};
    const errcode = body.errcode || body.error_code || 0;
    const ssaCode = body.ssaCode || (e.headers && e.headers['ssa-code']) || '';
    const edt = body.edt || '';
    const sid = body.sid || '';

    // SSA 二次验证：仅当确实有 ssaCode 时才触发
    if ((errcode === 20028 || ssaCode) && cookieObj.KUGOU_API_MID) {
      try {
        const verifyResult = await kgHandleSSA(
          cookieObj.KUGOU_API_MID,
          cookieObj.userid || 0,
          cookieObj.dfid || _kgCachedDfid,
          ssaCode,
          edt,
          sid,
          cookieObj
        ...
        if (verifyResult) {
          // 验证通过，重新注册设备获取新 dfid
          const newDfid = await kgRegisterDev();
          if (newDfid) {
            cookieObj.dfid = newDfid;
            // 同步更新 config.cookie（如果和 cookieObj 是同一引用或需要保持一致性）
            if (config.cookie) config.cookie.dfid = newDfid;
          }
          // 使用新 dfid 重试原始请求
          try {
            const retryResult = await kugou.createRequest({
              ...config,
              cookie: cookieObj,
            });
            return { data: retryResult.body, headers: retryResult.headers, cookie: retryResult.cookie };
          } catch (retryErr) {
            // 重试仍然失败，继续抛出
          }
        }
    ...
    // 将错误转换为可处理的格式
    throw Object.assign(new Error(body.error_msg || body.msg || body.error || e.message || 'kugou_request_failed'), {
      status: e.status || 502,
...
/** 酷狗歌曲 → Mineradio 统一歌曲格式 */
function mapKGSong(item) {
  // 酷狗不同 API 返回不同字段名，全面兼容：
  // 搜索: FileName, SingerName, FileHash, AlbumName, Duration
  // 歌单(/v4/get_list_all_file): filename(歌手 - 歌名.mp3格式), hash, duration
  // 公开歌单(/pubsongs): FileName, SingerName
...
/** 获取酷狗用户详细信息（昵称、头像、VIP状态） */
async function fetchKGUserInfo() {
  const obj = parseCookieString(kgCookie);
  if (!obj.token || !obj.userid) return null;
  const cookieObj = kgCookieObj();
  try {
    ...
    const r = await kugouRequest({
      url: '/v3/get_my_info',
      method: 'POST',
      data: { visit_time: clienttime_ms, usertype: 1, p: pk, userid: Number(obj.userid) },
      params: { plat: 1 },
      encryptType: 'android',
      cookie: cookieObj,
    ...
    const avatar = d.pic || d.k_pic || d.fx_pic || d.avatar || d.headimg || d.img || '';
    return {
      nickname: d.nickname || d.k_nickname || d.username || '',
      avatar: avatar,
      vipType: Number(d.vip_type || d.vipType || 0) || 0,
      isVip: (Number(d.vip_type || d.vipType || 0) || 0) > 0,
    };
  } catch (e) {
    console.warn('[KGUserInfo]', e.message);
    return null;
  }
}

/** 如果已登录但无用户信息，后台补充拉取 */
function ensureKGUserInfo() {
  if (kgLoginInfo.loggedIn && (!kgLoginInfo.nickname || kgLoginInfo.nickname === '酷狗用户')) {
    fetchKGUserInfo().then(info => {
      if (info) {
        kgLoginInfo.nickname = info.nickname || kgLoginInfo.nickname;
        kgLoginInfo.avatar = info.avatar || kgLoginInfo.avatar;
        kgLoginInfo.vipType = info.vipType || 0;
        kgLoginInfo.vipLevel = info.isVip ? 'vip' : 'none';
        kgLoginInfo.isVip = info.isVip;
      }
    }).catch(e => console.warn('[KGUserInfo]', e.message));
  }
}
...
const server = http.createServer(async (req, res) => {
    ...
    try {
      const r = await kugouRequest({
        ...
        params: {
          ...
          qrcode_txt: 'https://h5.kugou.com/apps/loginQRCode/html/index.html?appid=' + kugou.getAppid() + '&',
          srcappid: 2919,
        },
        encryptType: 'web',
        cookie: kgCookieObj(),
      ...
      const body = r.data || {};
      const key = (body.data && body.data.qrcode) || body.qrcode || '';
      sendJSON(res, { key });
    } catch (err) {
  ...
  if (pn === '/api/kugou/login/qr/check') {
    try {
      ...
      if (status === 4) {
        // 登录成功 — 从 cookie 或 body 提取 token/userid（保持原始逻辑）
        const token = data.token || '';
        const userid = data.userid || '';
        if (token && userid) {
          saveKGCookie('token=' + token + ';userid=' + userid);
          updateKGLoginInfoFromCookie();
          result.loggedIn = true;
          result.token = token;
          result.userid = userid;
          result.nickname = kgLoginInfo.nickname;
        } else if (r.cookie && r.cookie.length) {
          saveKGCookie(r.cookie.join('; '));
          updateKGLoginInfoFromCookie();
          result.loggedIn = kgLoginInfo.loggedIn;
        }

        // 异步：注册设备获取真实 dfid + 获取用户详细资料
        if (result.loggedIn) {
          kgRegisterDev().catch(e => console.warn('[KGRegisterDev]', e.message));
          fetchKGUserInfo().then(info => {
            if (info) {
              kgLoginInfo.nickname = info.nickname || kgLoginInfo.nickname;
              kgLoginInfo.avatar = info.avatar || kgLoginInfo.avatar;
  ...
  // -------- 酷狗概念版切换 --------
  if (pn === '/api/kugou/platform') {
    const mode = url.searchParams.get('mode') || '';
    if (mode === 'lite' || mode === 'standard' || mode === '') {
      saveKGPlatform(mode);
      sendJSON(res, { ok: true, platform: mode || 'standard', appid: kugou.getAppid(), clientver: kugou.getClientver() });
    } else {
      sendJSON(res, { ok: false, platform: process.env.platform || 'standard', appid: kugou.getAppid(), clientver: kugou.getClientver() });
    }
    return;
  }
  ...
  if (pn === '/api/kugou/login/cookie') {
    try {
      ...
      const obj = parseCookieString(normalized);
      if (!obj.token || !obj.userid) {
        sendJSON(res, { provider: 'kugou', loggedIn: false, error: 'INVALID_KG_COOKIE', message: '酷狗 cookie 缺少 token 或 userid' }, 400);
        return;
      }
      saveKGCookie(normalized);
      updateKGLoginInfoFromCookie();
      sendJSON(res, { ...kgLoginInfo, saved: true, hasCookie: !!kgCookie, provider: 'kugou' });
    } catch (err) {
  ...
  if (pn === '/api/kugou/login/status') {
    ...
    sendJSON(res, {
      ...
      hasCookie: !!kgCookie,
      provider: 'kugou',
      platform: isLite ? 'lite' : 'standard',
      appid: kugou.getAppid(),
      clientver: kugou.getClientver(),
    });
    return;
  }
  ...
  if (pn === '/api/kugou/register/dev') {
    try {
      const dfid = await kgRegisterDev();
      sendJSON(res, { provider: 'kugou', ok: true, dfid: dfid || '' });
    } catch (err) {
      console.error('[KGRegisterDev]', err);
  ...
  if (pn === '/api/kugou/logout') {
    saveKGCookie('');
    kgLoginInfo = { loggedIn: false, nickname: '酷狗音乐', userId: '', avatar: '', vipType: 0, vipLevel: 'none', isVip: false };
  ...
  if (pn === '/api/kugou/search') {
    try {
      ...
      const limit = Math.max(6, Math.min(50, parseInt(url.searchParams.get('limit') || '30', 10) || 30));
      ...
      const r = await kugouRequest({
        ...
        method: 'GET',
        params: {
          keyword: kw, page: 1, pagesize: limit,
          albumhide: 0, iscorrection: 1, nocollect: 0,
          platform: 'AndroidFilter',
        },
        encryptType: 'android',
      ...
      const lists = (data.lists || data.info || []).map(mapKGSong);
      sendJSON(res, { provider: 'kugou', songs: lists, total: data.total || lists.length });
    } catch (err) {
  ...
  if (pn === '/api/kugou/song/url') {
    try {
      ...
      const albumAudioId = url.searchParams.get('album_audio_id') || '0';
      const rawQuality = url.searchParams.get('quality') || '128';
      // 将前端网易云格式的音质转为酷狗格式
      ...
      const quality = qualityMap[rawQuality.toLowerCase()] || rawQuality;
      if (!hash) { sendJSON(res, { provider: 'kugou', error: 'Missing hash', url: '' }, 400); return; }
      const cookieObj = kgCookieObj();
      const isLite = kugou.isLite();

      // 平台参数（只用当前平台，跨平台 fallback 会导致 illegal key 31833）
      const plat = isLite
      ...
      const qualityChain = quality !== '128' ? [quality, '128'] : ['128'];
      ...
      for (const q of qualityChain) {
        if (songUrl) break;
        const baseParams = {
          hash, album_id: Number(albumId), album_audio_id: Number(albumAudioId),
          quality: q, behavior: 'play', pid: plat.pid, cmd: 26, pidversion: 3001,
          area_code: 1, ssa_flag: 'is_fromtrack', version: 11430,
          page_id: plat.page_id, ppage_id: plat.ppage_id,
          cdnBackup: 1, module: '', clientver: 11430,
        };
        for (const isFree of [0, 1]) {
          if (songUrl) break;
          try {
            // 概念版使用持久化 dfid（随机 dfid 会导致请求被拒）
            const requestDfid = isLite ? (cookieObj.dfid || _kgCachedDfid) : kugou.randomString(24);
            ...
            const r = await kugouRequest({
              ...
              params: { ...baseParams, IsFreePart: isFree },
              cookie: songCookie, headers: { 'x-router': 'trackercdn.kugou.com' },
            });
            const body = (r && r.data) || {};
            if (!finalUrlData.url) finalUrlData = body; // 保留第一个响应数据
            if (body.url && (Array.isArray(body.url) ? body.url[0] : body.url)) {
              songUrl = Array.isArray(body.url) ? body.url[0] : body.url;
            }
            if (!songUrl && body.backupUrl && Array.isArray(body.backupUrl) && body.backupUrl[0]) {
              songUrl = body.backupUrl[0];
            }
          } catch (e) {
            // isFree 重试失败时忽略，继续下一个音质
          }
        }
      }
      if (!songUrl) {
        console.log('[KGSongUrl] empty URL for hash:', hash, 'isLite:', isLite, 'hasDfid:', !!cookieObj.dfid);
      }
      sendJSON(res, { provider: 'kugou', url: songUrl, urls: finalUrlData.url || [], bitrate: finalUrlData.bitrate || 0 });
    } catch (err) {
      console.error('[KGSongUrl]', err);
      sendJSON(res, { provider: 'kugou', error: err.message || 'url_failed', url: '' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kugou/search/lyric') {
    try {
      ...
      const albumAudioId = url.searchParams.get('album_audio_id') || '0';
      if (!hash && !keyword) { sendJSON(res, { provider: 'kugou', error: 'Missing hash/keyword' }, 400); return; }
      const r = await kugouRequest({
        baseURL: 'https://lyrics.kugou.com', url: '/v1/search', method: 'GET',
        params: { album_audio_id: Number(albumAudioId), appid: kugou.getAppid(), clientver: kugou.getClientver(), duration: Number(duration), hash, keyword, lrctxt: 1, man: 'no' },
        encryptType: 'android', clearDefaultParams: true, notSign: true, cookie: kgCookieObj(),
      });
      const data = (r && r.data && r.data.data) || (r && r.data) || {};
      // 优先 candidates（歌词搜索），其次 info（可能是状态消息"OK"），最后 list
      const candidates = (Array.isArray(data.candidates) ? data.candidates : [])
        .concat(Array.isArray(data.ugccandidates) ? data.ugccandidates : []);
      const lyricInfo = candidates[0] || {};
      sendJSON(res, { provider: 'kugou', id: lyricInfo.id || '', accesskey: lyricInfo.accesskey || '', content: lyricInfo.content || '' });
    } catch (err) {
      console.error('[KGSearchLyric]', err);
  ...
  if (pn === '/api/kugou/lyric') {
    try {
      ...
      let accesskey = url.searchParams.get('accesskey') || '';
      const hash = url.searchParams.get('hash') || '';
      // 如果传了 hash 但没有 id/accesskey，自动先搜索歌词
      if (hash && (!id || !accesskey)) {
        const sr = await kugouRequest({
          baseURL: 'https://lyrics.kugou.com', url: '/v1/search', method: 'GET',
          params: { album_audio_id: 0, appid: kugou.getAppid(), clientver: kugou.getClientver(), duration: 0, hash, keyword: '', lrctxt: 1, man: 'no' },
          encryptType: 'android', clearDefaultParams: true, notSign: true, cookie: kgCookieObj(),
        });
        const sd = (sr && sr.data && sr.data.data) || (sr && sr.data) || {};
        const candidates = (Array.isArray(sd.candidates) ? sd.candidates : [])
          .concat(Array.isArray(sd.ugccandidates) ? sd.ugccandidates : []);
        const info = candidates[0] || {};
        id = info.id || id;
        accesskey = info.accesskey || accesskey;
      }
      if (!id || !accesskey) { sendJSON(res, { provider: 'kugou', error: 'Missing id/accesskey', lyric: '' }, 400); return; }
      const r = await kugouRequest({
        baseURL: 'https://lyrics.kugou.com',
        url: '/download',
        method: 'GET',
        params: { ver: 1, client: 'android', id, accesskey, fmt: 'krc', charset: 'utf8' },
        encryptType: 'android',
        cookie: kgCookieObj(),
      });
      const body = r && r.data;
      ...
      if (body && body.content) {
        rawLyric = kugou.decodeLyrics(body.content) || Buffer.from(body.content, 'base64').toString();
      }
      // KRC → LRC 转换：去除 <0,127,1> 逐字标签，[ms,dur] → [mm:ss.xx]
      let lrc = rawLyric.replace(/<[^>]*>/g, '');
      lrc = lrc.replace(/\[(\d+),(\d+)\]/g, function(_, ms) {
        var m = Math.floor(parseInt(ms) / 60000);
        var s = ((parseInt(ms) % 60000) / 1000).toFixed(2);
        return '[' + String(m).padStart(2,'0') + ':' + String(s).padStart(5,'0') + ']';
      });
      sendJSON(res, { provider: 'kugou', yrc: lrc, lyric: lrc, id, accesskey });
    } catch (err) {
  ...
  if (pn === '/api/kugou/user/playlists') {
    try {
      if (!kgLoginInfo.loggedIn) { sendJSON(res, { provider: 'kugou', loggedIn: false, playlists: [] }); return; }
      const cookieObj = kgCookieObj();
      const userid = cookieObj.userid || kgLoginInfo.userId || 0;
      const token = cookieObj.token || '';
      const r = await kugouRequest({
      ...
      if (allIds.length) {
        try {
          const coverMap = await kgFetchPlaylistCovers(allIds);
          if (coverMap) {
            list.forEach(pl => {
              const realCover = coverMap[pl.id];
              if (realCover) pl.cover = realCover;
            });
          }
        } catch (e) { /* 封面获取失败不影响歌单列表 */ }
      }

      sendJSON(res, { provider: 'kugou', loggedIn: true, userId: kgLoginInfo.userId, playlists: list });
    } catch (err) {
  ...
  if (pn === '/api/kugou/playlist/detail') {
    try {
      ...
      if (!ids.length) { sendJSON(res, { provider: 'kugou', error: 'Missing ids', playlists: [] }, 400); return; }
      const cookieObj = kgCookieObj();
      const r = await kugouRequest({
        url: '/v3/get_list_info', method: 'POST',
        data: { data: ids.map(s => ({ global_collection_id: s })), userid: Number(cookieObj.userid || 0), token: cookieObj.token || '' },
      ...
      const body = (r && r.data) || {};
      const raw = body.data || body.info || [];
      const list = (Array.isArray(raw) ? raw : Object.values(raw)).filter(v => v && typeof v === 'object' && v.global_collection_id).map(pl => ({
      ...
      sendJSON(res, { provider: 'kugou', playlists: list });
    } catch (err) {
      console.error('[KGPlaylistDetail]', err);
  ...
  if (pn === '/api/kugou/playlist/tracks') {
    try {
      ...
      const token = cookieObj.token || '';
      const useListid = listid || (/^\d+$/.test(id) ? id : '');
      ...
      sendJSON(res, { provider: 'kugou', tracks, total: tracks.length });
    } catch (err) {
  ...
  // ---------- 酷狗：获取歌单所有歌曲（by global_collection_id）----------
  if (pn === '/api/kugou/playlist/track/all') {
    try {
      const id = url.searchParams.get('id') || '';
      ...
      const r = await kugouRequest({
        url: '/pubsongs/v2/get_other_list_file_nofilt', method: 'GET',
        params: {
          global_collection_id: id, page, pagesize, area_code: 1, plat: 1,
          type: 1, mode: 1, personal_switch: 1,
  ...
  if (pn === '/api/kugou/playlist/track/all/new') {
    try {
      ...
      const result = await kugou.createRequest({
        url: '/v4/get_list_all_file', method: 'POST',
        data: { listid, userid, area_code: 1, show_relate_goods: 0, pagesize, allplatform: 1, show_cover: 1, type: 0, token, page },
        encryptType: 'android', cookie: cookieObj,
        headers: { 'x-router': 'cloudlist.service.kugou.com' },
      });
      ...
      console.error('[KGTrackAllNew]', err);
      sendJSON(res, { provider: 'kugou', error: err.message, tracks: [] }, 500);
    }
    return;
  }
  ...
  // ---------- 酷狗：热门好歌精选（热歌榜单）----------
  if (pn === '/api/kugou/top/card') {
    try {
      const cardId = url.searchParams.get('card_id') || '3';
      const cookieObj = kgCookieObj();
      ...
      const r = await kugouRequest({
        ...
        data: {
          appid: kugou.getAppid(), clientver: kugou.getClientver(),
          platform: 'android', clienttime: dateTime,
          userid: Number(cookieObj.userid || 0),
          key: kugou.signParamsKey(dateTime),
          fakem: '60f7ebf1f812edbac3c63a7310001701760f',
          area_code: 1, mid: cookieObj.KUGOU_API_MID,
          uuid: '-', client_playlist: [],
          u_info: 'a0c35cd40af564444b5584c2754dedec',
        },
        params: { card_id: Number(cardId), fakem: '60f7ebf1f812edbac3c63a7310001701760f', area_code: 1, platform: 'ios' },
        cookie: cookieObj,
      });
      const body = (r && r.data) || {};
      const songs = (body.data && body.data.song_list || body.data || []).map(mapKGSong);
      sendJSON(res, { provider: 'kugou', songs, total: songs.length });
    } catch (err) {
      console.error('[KGTopCard]', err);
      sendJSON(res, { provider: 'kugou', error: err.message, songs: [] }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kugou/personal/fm') {
    try {
      ...
      const dateTime = Date.now();
      const dataMap = {
        appid: kugou.getAppid(), clienttime: dateTime,
        mid: cookieObj.KUGOU_API_MID,
        action: url.searchParams.get('action') || 'play',
        recommend_source_locked: 0,
        song_pool_id: Number(url.searchParams.get('song_pool_id') || 0),
        callerid: 0, m_type: 1,
        platform: url.searchParams.get('platform') || 'ios',
        area_code: 1,
        remain_songcnt: Number(url.searchParams.get('remain_songcnt') || 0),
        clientver: kugou.getClientver(),
        is_overplay: url.searchParams.get('is_overplay') ? 1 : 0,
        mode: url.searchParams.get('mode') || 'normal',
        fakem: 'ca981cfc583a4c37f28d2d49000013c16a0a',
        key: kugou.signParamsKey(dateTime),
      };
      const userid = Number(cookieObj.userid || 0);
      const token = cookieObj.token || '';
      const vipType = cookieObj.vip_type || 0;
      if (userid) { dataMap['userid'] = userid; dataMap['kguid'] = userid; }
      if (token) dataMap['token'] = token;
      if (vipType) dataMap['vip_type'] = vipType;
      if (url.searchParams.get('hash')) dataMap['hash'] = url.searchParams.get('hash');
      if (url.searchParams.get('songid')) dataMap['songid'] = url.searchParams.get('songid');
      if (url.searchParams.get('playtime')) dataMap['playtime'] = url.searchParams.get('playtime');

      const r = await kugouRequest({
        url: '/v2/personal_recommend', method: 'POST',
        data: dataMap, encryptType: 'android', cookie: cookieObj,
        headers: { 'x-router': 'persnfm.service.kugou.com' },
      });
      const body = (r && r.data) || {};
      const songList = (body.data && body.data.song_list) || [];
      const songs = (Array.isArray(songList) ? songList : []).map(function(s) {
        return {
          id: (s.hash || '').toLowerCase(),
          name: s.songname || s.filename || '',
          artist: (s.singerinfo || []).map(function(a){ return a.name || a.singername || ''; }).filter(Boolean).join(' / ') || s.singername || '',
          hash: (s.hash || '').toLowerCase(),
          album: s.album_name || s.remark || '',
          cover: '',
      ...
      console.error('[KGPersonalFM]', err);
      sendJSON(res, { provider: 'kugou', error: err.message }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kugou/top/song') {
    try {
      ...
      const r = await kugouRequest({
        ...
        encryptType: 'android', method: 'POST',
        data: {
          rank_id: Number(url.searchParams.get('type') || 21608),
          userid: Number(cookieObj.userid || 0),
      ...
      const body = (r && r.data) || {};
      const songs = (body.data || []).map(mapKGSong);
      sendJSON(res, { provider: 'kugou', songs, total: songs.length });
    } catch (err) {
      console.error('[KGTopSong]', err);
      sendJSON(res, { provider: 'kugou', error: err.message, songs: [] }, 500);
    }
    return;
  }
  ...
  // ---------- 酷狗：收藏歌单 / 新建歌单 ----------
  if (pn === '/api/kugou/playlist/add') {
    try {
      const body = await readRequestBody(req);
      const cookieObj = kgCookieObj();
      const userid = body.userid || url.searchParams.get('userid') || cookieObj.userid || 0;
      const token = body.token || url.searchParams.get('token') || cookieObj.token || '';
      const clienttime = Math.floor(Date.now() / 1000);
      const type = parseInt(body.type || url.searchParams.get('type') || '0', 10);
      ...
      const dataMap = {
        ...
        type,
        source: body.source || url.searchParams.get('source') || 1,
        is_pri: 0,
        list_create_userid: body.list_create_userid || url.searchParams.get('list_create_userid') || '',
        list_create_listid: body.list_create_listid || url.searchParams.get('list_create_listid') || '',
        list_create_gid: body.list_create_gid || url.searchParams.get('list_create_gid') || '',
        from_shupinmv: 0,
      };

      if (type === 0) {
        dataMap['is_pri'] = parseInt(body.is_pri || url.searchParams.get('is_pri') || '0', 10) || 0;
      }
      ...
      console.error('[KGPlaylistAdd]', err);
      sendJSON(res, { provider: 'kugou', error: err.message }, 500);
    }
    return;
  }
  ...
  // ---------- 酷狗：取消收藏歌单 / 删除歌单 ----------
  if (pn === '/api/kugou/playlist/del') {
    try {
      const cookieObj = kgCookieObj();
      const userid = url.searchParams.get('userid') || cookieObj.userid || 0;
      const token = url.searchParams.get('token') || cookieObj.token || '';
      const clienttime = Math.floor(Date.now() / 1000);
      const listid = url.searchParams.get('listid') || '';

      if (!listid) { sendJSON(res, { provider: 'kugou', error: 'Missing listid' }, 400); return; }
      ...
      const r = await kugouRequest({
        ...
        params: {
          clienttime,
          key: kugou.signParamsKey(clienttime.toString()),
          last_area: 'gztx',
          clientver: kugou.getClientver(),
          appid: kugou.getAppid(),
      ...
      const decrypted = kugou.playlistAesDecrypt({ str: Buffer.from(r.data).toString('base64'), key: aesEncrypt.key });
      sendJSON(res, { provider: 'kugou', data: decrypted });
    } catch (err) {
      console.error('[KGPlaylistDel]', err);
      sendJSON(res, { provider: 'kugou', error: err.message }, 500);
    }
    return;
  }
  ...
  // ---------- 酷狗：对歌单添加歌曲 ----------
  if (pn === '/api/kugou/playlist/tracks/add') {
    try {
      const cookieObj = kgCookieObj();
      const userid = url.searchParams.get('userid') || cookieObj.userid || 0;
      const token = url.searchParams.get('token') || cookieObj.token || '';
      ...
      if (!listid || !rawData) { sendJSON(res, { provider: 'kugou', error: 'Missing listid or data' }, 400); return; }

      const resource = rawData.split(',').map((s) => {
        const d = s.split('|');
        return {
          number: 1,
          name: d[0] || '',
          ...
          timelen: 0,
          bitrate: 0,
          album_id: Number(d[2] || 0),
          mixsongid: Number(d[3] || 0),
        };
      });
      ...
      console.error('[KGPlaylistTracksAdd]', err);
      sendJSON(res, { provider: 'kugou', error: err.message }, 500);
    }
    return;
  }
  ...
  // ---------- 酷狗：对歌单删除歌曲 ----------
  if (pn === '/api/kugou/playlist/tracks/del') {
    try {
      const cookieObj = kgCookieObj();
      const userid = url.searchParams.get('userid') || cookieObj.userid || 0;
      const token = url.searchParams.get('token') || cookieObj.token || '';
      ...
      if (!listid || !fileids) { sendJSON(res, { provider: 'kugou', error: 'Missing listid or fileids' }, 400); return; }

      const resource = fileids.split(',').map((s) => ({ fileid: Number(s.trim()) }));
      ...
      console.error('[KGPlaylistTracksDel]', err);
      sendJSON(res, { provider: 'kugou', error: err.message }, 500);
    }
    return;
  }
  ...
  // ---------- 酷狗概念版：升级 VIP ----------
  if (pn === '/api/kugou/youth/day/vip/upgrade') {
    try {
      const cookieObj = kgCookieObj();
      const userid = url.searchParams.get('userid') || cookieObj.userid || 0;
      const r = await kugouRequest({
...
// 供桌面主进程同步 cookie（避免文件轮询延迟）
function syncKGCookieFromFile() {
  try {
    if (fs.existsSync(KG_COOKIE_FILE)) {
      const fresh = fs.readFileSync(KG_COOKIE_FILE, 'utf8').trim();
      if (fresh && fresh !== kgCookie) {
        kgCookie = fresh;
        updateKGLoginInfoFromCookie();
        console.log('[KG Sync] cookie synced, loggedIn:', kgLoginInfo.loggedIn, 'nickname:', kgLoginInfo.nickname);
        if (kgLoginInfo.loggedIn && (!kgLoginInfo.nickname || kgLoginInfo.nickname === '酷狗用户')) {
          setTimeout(() => ensureKGUserInfo(), 500);
        }
      }
    }
  } catch (_) {}
}
server.syncKGCookie = syncKGCookieFromFile;
